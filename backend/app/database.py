"""SQLite database setup with auth-enabled schema."""
...
def init_db():
    ...
    # Create core tables (without auth columns in case old DB exists)
    conn.executescript("""
    ...
    # Now safe to create the token index
    conn.execute("CREATE INDEX IF NOT EXISTS idx_users_token ON users(auth_token)")

    # Make email NOT NULL for new schema (existing rows keep their values)
    conn.commit()
    conn.close()
